...
//		Animal a1 = new Animal();
	...
	public void ex2() {
		...
		// 추상 클래스는 객체로 만들 수 없다
		// -> 하지만 "참조 변수"로는 사용할 수 있음
		
//		Animal a1 = new Animal(): // 오류 발생
		
		// 사람 -> 동물 / 물고기 -> 동물
//		Animal a1 = new Person(); // 사용 가능 / 다형성 중 업캐스팅 적용
//		Animal a2 = new Fish(); // 사용 가능 / 다형성 중 업캐스팅 적용
		
		Animal[] arr = new Animal[2]; // Animal 참조변수 배열 선언 및 할당
		
		arr[0] = new Person("사람", "잡식", "김사람"); // Animal 부모 = Person 자식(다형성 중 업캐스팅)
		arr[1] = new Fish("물고기", "잡식"); // Animal 부모 = Fish 자식(다형성 중 업캐스팅)
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// 메서드 수행
			// -> 동적 바인딩 : 부모 타입 참조변수로 메서드를 호출했지만
			//					자식 타입의 오버라이딩된 메서드가 수행됨
