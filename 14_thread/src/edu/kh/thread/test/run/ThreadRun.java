...
//		ThreadWithRunnable thread2 = new ThreadWithRunnable(); // XXX
		Thread thread2 = new Thread(new ThreadWithRunnable());
		// Runnable 인터페이스에서 run 메서드 하나만읠 정의하고 있기 때문에
		// start나, stop과 같은 Thread 메서드를 사용할 수 없다.
		// -> Thread 클래스에 Runnable 인터페이스를 상속받아 구현한
		// ThreadWithRunnable 객체를 전달하여 사용해야 한다.

		thread1.start(); // 스레드의 실행 (클래스 상속)
		thread2.start(); // 스레드의 실행 (인터페이스 상속)
