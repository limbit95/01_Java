...
public class SetService {
	...
	// Set(집합, 주머니)
	// - 기본적으로 순서를 유지하지 않음
	//	 (index 없음 -> get() 메서드도 없음)
	// - 중복 데이터 저장X (같은 객체가 들어오면 덮어쓰기)
...
//		Set<String> set = new LinkedHashSet<String>(); // 다형성-업캐스팅
	...
	public void method2() {
		...
		// Iterator를 이용한 for문으로도 출력이 가능하나
		// 주의할 점은 Iterator.next() 는 한 번 사용하면 이후에 또 사용할 때
		// 가장 마지막에 호출했던 요소 다음부터 또 호출하기 때문에 해당 set에 있는
		// 모든 요소를 다 호출한 상태에서 Iterator.next()를 호출하면 요소가 없다며 예외 발생
...
//		}
	...
	public void method4() {
		...
		// == 6개 멈춤
		while(lotto.size() < 6) {
			// random.nextInt(45) : 0 <= x < 45 의 난수를 뜻함
