...
//			}
		...
		} finally { // try 예외 발생 여부 관계 없이 무조건 수행하는 구문
	...
	public void bufferedFileByteOutput() {
		...
		// -> 동영상 재생 중에 앞에 재생될 부분을 
		//    미리 다운 받아놔서 영상을 끊김없이 볼 수 있게 함
		//    -> 다운 받아서 저장해놓는 공간 == 버퍼
		
		// BufferedOutputStream / BufferedInputStream
	
		// - FileOutputStream은 1바이크씩 데이터를 입출력
		...
		try {
			...
			try {
				if(bos != null) {
					// 보조스트림 close()시
					// 보조스트림 생성에 사용된 기반 스트림도 같이 close() 됨
					bos.close();
					System.out.println("출력 완료");
				}
			} catch (IOException e) {
	...
	public void fileByteInput() {
		...
		try {
			...
			// InputStream은 flush() 없음 외부에서 내부로 읽어오는 것이기 때문에
	 ...
	 * 
	 * [실행화면]
	 * 파일 경로 입력 : /io_test/202402222/노래가사.txt
	 * 복사 완료 : /io_test/202402222/노래가사_copy.txt
	 * 
	 * 1) 입력된 경로에 파일이 있는지 검사
	 * 2) 있으면 파일 내용을 모두 읽어오기 : FileInputStream + BufferedInputStream
	 * 3) 읽어온 내용을 같은 위치에 "파일명_copy" 이름으로 출력
	 *	  : FileOutputStream + BuffetedOutputStream
	 *
	...
	public void fileCopy() {
		...
		try {
			...
			StringBuilder sb = new StringBuilder();
			sb.append(target); // /io_test/202402222/노래가사.txt
			...
			sb.insert(insertPoint, "_copy"); // /io_test/202402222/노래가사_copy.txt
